...
/*   Updated: 2023/04/20 22:07:01 by hakahmed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fractol	init(char *name)
{
	...
	res.height = 600;
	res.mlx = mlx_init();
	res.win = mlx_new_window(res.mlx, res.width, res.height, name);
	...
	res.rotate = -1;
	res.color = 1;
	res.available_fractals = ft_split("Mandelbrot Julia Multibrot", 32);
	return (res);
}
